...
const addProductReducer = (state = initialState, action) => {
  if (action.type === ADD_PRODUCT) {
    ...
    let products = state.products.slice();
    products[action.payload.productIndex].ingredients.push(
    ...
    let products = state.products.slice();
    let id =
      products[action.payload.productIndex].ingredients[action.payload.index]
        ._id;
    if (id === action.payload.id)
      products[action.payload.productIndex].ingredients.splice(
        action.payload.index,
        1
      );
    else {
      let index = products[action.payload.productIndex].ingredients.findIndex(
        ingredient => {
    ...
    let products = state.products.slice();
    products[action.payload.productIndex].ingredients[
  ...
  } else if (action.type === SET_REPRESENTATIVE_SIGNATURE) {
    return {
      ...state,
      representativeSignature: action.payload
    };
  } else if (action.type === SET_AFFIDAVIT_BLOB) {
    return {
      ...state,
      affidavitBlob: action.payload
    };
  } else if (action.type === SET_ACCEPT_TERMS) {
    return {
      ...state,
      acceptTerm: action.payload
    };
  }
  return state;
};
